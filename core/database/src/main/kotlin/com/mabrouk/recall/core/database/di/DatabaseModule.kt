package com.mabrouk.recall.core.database.di

import android.content.Context
import androidx.room.Room
import com.mabrouk.recall.core.database.RecallDatabase
import com.mabrouk.recall.core.database.dao.NoteDao
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.android.qualifiers.ApplicationContext
import dagger.hilt.components.SingletonComponent
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
object DatabaseModule {

    @Provides
    @Singleton
    fun provideRecallDatabase(
        @ApplicationContext context: Context,
    ): RecallDatabase {
        // Pre-release: destructive fallback is OK until we ship real migrations.
        return Room.databaseBuilder(
            context,
            RecallDatabase::class.java,
            RecallDatabase.DATABASE_NAME,
        )
            .fallbackToDestructiveMigration(dropAllTables = true)
            .build()
    }

    @Provides
    fun provideNoteDao(database: RecallDatabase): NoteDao = database.noteDao()
}
